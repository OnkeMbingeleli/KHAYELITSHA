...
// Every admin.* server function now requires a verified staff access_token.
// This grabs the current Supabase session token right before each call.
...
function AdminSubheads() {
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const r = await list({ data: { access_token } });
      setOffices(r.offices as any);
      setRoutes(r.routes as any);
      setSessions(r.sessions as any);
    } catch (err: any) {
      toast.error(err?.message ?? "Failed to load subhead offices");
  ...
  const resetCode = async (id: string) => {
    ...
    try {
      const access_token = await getToken();
      await update({ data: { access_token, id, code: c } });
  ...
  const remove = async (id: string) => {
    ...
    try {
      const access_token = await getToken();
      await del({ data: { access_token, id } });
  ...
  const kickSession = async (id: string) => {
    try {
      const access_token = await getToken();
      await kick({ data: { access_token, id } });
      refresh();
    } catch (err: any) { toast.error(err?.message ?? "Failed"); }
  };
                  ...
                  <ul className="text-xs text-muted-foreground mt-1 space-y-1">
                    {sess.map((s) => (
                      <li key={s.id} className="flex items-center gap-2">
                        ...
                        <button onClick={() => kickSession(s.id)} className="text-destructive hover:underline">
