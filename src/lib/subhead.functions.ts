...
// =========================================================
// SECURITY: every admin.* function below now requires a valid
// Supabase access_token belonging to a super_admin/management
// user. Without this, the server function endpoint itself was
// callable by anyone, regardless of what the React UI hides.
// =========================================================
async function requireStaff(accessToken: string | undefined) {
  ...
  if (rolesErr) throw new Error("Could not verify role");

  const isStaff = (roles ?? []).some((r) => r.role === "super_admin" || r.role === "management");
...
export const subheadHeartbeat = createServerFn({ method: "POST" })
  .validator((d) => z.object({ device_token: z.string().min(10) }).parse(d))
  .handler(async ({ data }) => {
...
// ===== Admin (all require a verified staff access_token) =====

export const adminListOffices = createServerFn({ method: "POST" })
  .validator((d) => z.object({ access_token: z.string() }).parse(d))
  .handler(async ({ data }) => {
    await requireStaff(data.access_token);
...
export const adminDeleteOffice = createServerFn({ method: "POST" })
  .validator((d) => z.object({ access_token: z.string(), id: z.string().uuid() }).parse(d))
  .handler(async ({ data }) => {
    await requireStaff(data.access_token);
